...
function StreamCard({ camera, index }: { camera: Camera; index: number }) {
    ...
    >
      ...
      <div className="relative aspect-video overflow-hidden bg-black/40">
        <Image
          ...
          alt={camera.name}
          fill
          className="object-cover transition-transform duration-300 ease-cyber group-hover:scale-105"
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-3 pt-8">
          <h3 className="text-sm font-semibold text-white truncate leading-tight font-sans">
            {camera.name}
          </h3>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-mono font-bold text-foreground uppercase tracking-[0.15em]">
            Live Stream Matrix
          </h2>
        </div>
      ...
      >
        {mockCameras.map((camera, index) => (
          <StreamCard key={camera.id} camera={camera} index={index} />
        ))}
      </div>
    </div>
  )
